...
private static void printSolution(int[] dist,String[] pCamino ,int n, int pNodoInicio)
{
   System.out.println("Caminos mas cortos \n");
   for (int i = 0; i < V; i++)
      System.out.println("El camino de "+ pNodoInicio + " hasta "+ i + " es "+ pCamino[i] +" y cuesta "+dist[i]);
}
 
private static void dijkstra(int[][] grafo, int src)
{
     int    [] dist     = new int[V]; 
     String [] camino   = new String[V];
     ...
     for (int count = 0; count < V-1; count++)
     {
       ...
       for (int v = 0; v < V; v++)
                                       ...
                                       && dist[u]+grafo[u][v] < dist[v])
         {
            ...
            camino[v] = camino[u] + "-> " + Integer.toString(v);
         }
            
     }
 
     // se imprime el arreglo con las distancias
     printSolution(dist, camino ,V,src);
}
